...
<html lang="en">
  ...
  <body>
        ...
        foreach($consulta as $registros)
      {
        ...
        <form action="VerOrden.php" method="POST" class="tamano"enctype="multipart/form-data">
                     <div class="col-lg-7">
                      <label for="nombre" class="form-label "><strong>Fecha en la que se hizo la entrega del producto</strong></label>
                      <input  required class="form-control" name="fecha_detalle1"type="date" />
                    </div>
                        <br>
                    <div class="col-lg-5">
                      <label for="precio" class="form-label "><strong>Precio al que se vendio el producto</strong></label>
                      <input required type="text" name="precio" id="" class="form-control" placeholder="Escribe el precio del producto aqui">
                    </div>
                    <input type="hidden" name="orden" value="<?php echo $orden; ?>">
                    <br><br><br><br><br><br>
                    <div class="col-lg-12">
                <button type="submit" class="btn btn-primary tamano">Confirmar Orden</button>
      </div>
        </form>
        </div>
        <?php
        
                if (isset($_POST['fecha_detalle1']) && isset($_POST['precio']) && isset($_POST['orden']))
                {
                    $update = new Ejecuta();
                    $ordenDet=$_POST['orden'];
                    $precio=$_POST['precio'];
                    $fecha=$_POST['fecha_detalle1'];
                    $insercion= "UPDATE detalle_orden SET `precio_venta`=$precio and `fecha_detalle`='$fecha' WHERE `orden`=$orden";
